...
├── api/                ← Vercel Serverless Functions (Node.js 20, ESM)
│   ├── _lib/
│   │   └── utils.js    ← Общие утилиты: verifyTelegramInitData, getGoogleAccessToken
│   ├── webhook.js      ← Telegram Bot: /start, /help, /contact, произвольные сообщения
│   ├── save-result.js  ← Стадия 1: сохранить прохождение в Sheets + TG-сообщение пользователю
│   ├── lead.js         ← Стадия 2: форма лида → amoCRM + Sheets + TG-уведомление команде
│   ├── check-sub.js    ← Проверить подписку пользователя на канал @medsprint
│   └── send-material.js← Отправить PDF-карту каналов пользователю через бота
├── research.md         ← Исходное исследование рынка и конкурентов
├── brief.md            ← Детальный бриф продукта (экраны, элементы, логика)
├── BACKEND-PLAN.md     ← Архитектура бэкенда и план реализации
├── TESTING.md          ← Руководство тестировщика: все тест-кейсы
├── DEPLOYMENT.md       ← Пошаговая инструкция деплоя (актуальная)
└── CLAUDE.md           ← Этот файл
```
...
[screen-result]     Результат: балл, карта рисков, топ-3 проблемы
                    + встроенная карточка лидмагнита (подписка → PDF в бот)
    ↓
...
**Лидмагнит на экране результата:** карточка предлагает карту каналов привлечения пациентов (PDF). Флоу: кнопка → тихая проверка подписки (`/api/check-sub`) → если не подписан, открывает канал @medsprint → «Я подписался» → серверная проверка → `/api/send-material` отправляет PDF через бота → кнопка меняется на бейдж «📨 Карта отправлена в Telegram».
...
const CONFIG = {
  ...
  totalAudited: 146,                 // счётчик на стартовом экране
...
- PDF-отчёт с логотипом клиники (персонализированный)
